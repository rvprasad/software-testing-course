...
class BST(object):
    ...
    def __print(node, indent, type):
        if node:
            print("{0}{1}".format(type, node._key))
            BST.__print(node._left, indent + 1, "{0}l".format(type))
            BST.__print(node._right, indent + 1, "{0}r".format(type))
    
    def print(self):
        BST.__print(self._root, 0, "*")
