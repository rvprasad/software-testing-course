...
class BST(object):
    ...
    @staticmethod
    def __print(node, indent, type):
        if node: # pragma: no cover
            print("{0}{1}".format(type, node._key)) # pragma: no cover
            BST.__print(node._left, indent + 1, "{0}l".format(type)) # pragma: no cover
            BST.__print(node._right, indent + 1, "{0}r".format(type)) # pragma: no cover

    def print(self):
        BST.__print(self._root, 0, "*") # pragma: no cover
