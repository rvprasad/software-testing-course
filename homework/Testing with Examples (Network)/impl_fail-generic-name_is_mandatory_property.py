...
class Network(object):
    ...
    def set_property(self, person, prop, value):
        self.node_properties[person][prop] = value
        
    def get_property(self, person, prop):
        return self.node_properties.get(person).get(prop)
        
    def has_property(self, person, prop):
        return prop in self.node_properties.get(person)
    ...
    def add_person_property(self, person, prop, value):
        ...
        if prop == Network.NAME_PROP:
            ...
            for p in self.nodes:
                if self.get_property(p, Network.NAME_PROP) == value and \
                    p is not person:
                    raise ValueError("{0} name already taken".format(value))
        self.set_property(person, prop, value)
    ...
    def add_relation_property(self, person1, person2, prop, value):
        if not self.has_property(person1, Network.NAME_PROP):
            raise RuntimeError("person1 without name")
        if not self.has_property(person2, Network.NAME_PROP):
            raise RuntimeError("person2 without name")
    ...
    def get_person(self, name):
        ...
        for n in self.nodes:
            if self.get_property(n, Network.NAME_PROP) == name:
                return n
        # flag non-existent person
