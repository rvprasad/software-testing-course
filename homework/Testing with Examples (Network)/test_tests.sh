...
#i="impl_fail-friends_of_friends-raises_wrong_exception_for_non_string_name.py"
  ...
  j="$j E$tmp1"
  k=`echo $i | cut -d'-' -f2`
  for l in "create_person" "add_person_property" \
    "add_relation" "add_relation_property" "get_person" "friends_of_friends" ; do 
    output $k $l
    j="$j $or"
  done
  echo $j
...
echo "The columns are correspond to the following methods: create_person add_person_property add_relation add_relation_property get_person friends_of_friends" 

pushd temp > /dev/null
grep "test_.*\(FAILED\|ERROR\)$" pytest-output-impl_* | sed -E "s/pytest-output-(.*).py.txt:test_impl.py::([^\[]*)(\[.*\])? (.*)/\2 \4 \1/" | sort -u > _temp1_

grep "test_.*\(FAILED\|ERROR\)$" pytest-output-impl_* | sed -E "s/pytest-output-(.*).py.txt:test_impl.py::([^\[]*)(\[.*\])? (.*)/\1 \4 \2/" | sort -u > _temp2_
